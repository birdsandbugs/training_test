...
bgchem <- read.csv(url(data_url, method = "libcurl"), stringsAsFactors = FALSE) %>% 
    ...
    verticalLayout(
        ...
        sidebarLayout(
            sidebarPanel(
                ...
                selectInput("y_variable", "Y Variable", cols, selected = "d180"),
...
server <- function(input, output) {
    ...
    output$varPlot <- renderPlot({
        ggplot(bgchem, mapping = aes_string(x = input$x_variable, 
                                            y = input$y_variable, 
            ...
            scale_color_gradient2(low = "midnightblue",
                                  mid = "white",
                                  high = "firebrick",
                                  midpoint = mean(bgchem[,input$color_variable])) + 
            theme_light()
    })
